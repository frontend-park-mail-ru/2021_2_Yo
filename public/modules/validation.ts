...
export function authValidateFields(
    inputsData: Map<string, { errors: string[]; value: string }>
) {
    const email = inputsData.get('email') as { errors: string[]; value: string };
    email.errors.push(checkEmail(email.value));

    const password = inputsData.get('password') as {
    errors: string[];
    value: string;
  };
...
export function signupValidateFields(
    inputsData: Map<string, { errors: string[]; value: string }>
) {
    const name = inputsData.get('name') as { errors: string[]; value: string };
    const surname = inputsData.get('surname') as {
    errors: string[];
    value: string;
  };
    const email = inputsData.get('email') as { errors: string[]; value: string };
    const password1 = inputsData.get('password1') as {
    errors: string[];
    value: string;
  };
    const password2 = inputsData.get('password2') as {
    errors: string[];
    value: string;
  };
...
export function eventValidateFields(
    ...
    const description = inputsData.get('description') as {
    errors: string[];
    value: string;
  };
    ...
    const category = inputsData.get('category') as {
    errors: string[];
    value: string;
  };
...
function checkInputLength(
    ...
    inputsData.forEach((item, key) => {
        if (item.value.length > (inputLength.get(key) as number)) {
            item.errors.push(
                `Слишком много символов. Максимальная длина ${
          inputLength.get(key) as number
                }`
            );
        }
    });
